...
Client certificates generated by [kubeadm](/docs/kubernetes/en/reference/setup-tools/kubeadm/) expire after 1 year.
...
You should be familiar with [PKI certificates and requirements in Kubernetes](/docs/kubernetes/en/setup/best-practices/certificates/).
...
[PKI certificates and requirements](/docs/kubernetes/en/setup/best-practices/certificates/) includes guidance on
...
for [automatic certificate renewal](/docs/kubernetes/en/tasks/tls/certificate-rotation/)
with rotatable certificates under `/var/lib/kubelet/pki`.
To repair an expired kubelet client certificate see
[Kubelet client certificate rotation fails](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/troubleshooting-kubeadm/#kubelet-client-cert).
{{< /note >}}
...
kubeadm renews all the certificates during control plane [upgrade](/docs/kubernetes/en/tasks/administer-cluster/kubeadm/kubeadm-upgrade/).
...
[Static Pods](/docs/kubernetes/en/tasks/configure-pod-container/static-pod/) are managed by the local kubelet
...
and wait for 20 seconds (see the `fileCheckFrequency` value in [KubeletConfiguration struct](/docs/kubernetes/en/reference/config-api/kubelet-config.v1beta1/).
...
The built-in signer is part of [`kube-controller-manager`](/docs/kubernetes/en/reference/command-line-tools-reference/kube-controller-manager/).
...
See [Create CertificateSigningRequest](/docs/kubernetes/en/reference/access-authn-authz/certificate-signing-requests/#create-certificatesigningrequest) for creating CSRs with the Kubernetes API.
...
It is the responsibility of the CA to specify [the correct cert usages](/docs/kubernetes/en/setup/best-practices/certificates/#all-certificates)
...
For more information about manual rotation or replacement of CA, see [manual rotation of CA certificates](/docs/kubernetes/en/tasks/tls/manual-rotation-of-ca-certificates/).
...
[`kubernetes.io/kubelet-serving`](/docs/kubernetes/en/reference/access-authn-authz/certificate-signing-requests/#kubernetes-signers).
...
[Certificate Rotation](/docs/kubernetes/en/reference/access-authn-authz/kubelet-tls-bootstrapping/#certificate-rotation).
...
[`system:masters`](/docs/kubernetes/en/reference/access-authn-authz/rbac/#user-facing-roles)
...
Instead, you can use the [`kubeadm kubeconfig user`](/docs/kubernetes/en/reference/setup-tools/kubeadm/kubeadm-kubeconfig)
command to generate kubeconfig files for additional users.
The command accepts a mixture of command line flags and
[kubeadm configuration](/docs/kubernetes/en/reference/config-api/kubeadm-config.v1beta3/) options.
