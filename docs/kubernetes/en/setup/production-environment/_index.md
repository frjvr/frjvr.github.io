...
- *Availability*: A single-machine Kubernetes [learning environment](/docs/kubernetes/en/setup/#learning-environment)
...
- *Security and access management*: You have full admin privileges on your own
  ...
  ([RBAC](/docs/kubernetes/en/reference/access-authn-authz/rbac/)) and other
  ...
  by managing [policies](/docs/kubernetes/en/concepts/policy/) and
  [container resources](/docs/kubernetes/en/concepts/configuration/manage-resources-containers/).

Before building a Kubernetes production environment on your own, consider
handing off some or all of this job to 
[Turnkey Cloud Solutions](/docs/kubernetes/en/setup/production-environment/turnkey-solutions/) 
...
[Kubernetes Components](/docs/kubernetes/en/concepts/overview/components/).
...
- *Choose deployment tools*: You can deploy a control plane using tools such
  ...
  [Installing Kubernetes with deployment tools](/docs/kubernetes/en/setup/production-environment/tools/)
  to learn tips for production-quality deployments using each of those deployment
  methods. Different [Container Runtimes](/docs/kubernetes/en/setup/production-environment/container-runtimes/)
...
- *Manage certificates*: Secure communications between control plane services
  ...
  See [PKI certificates and requirements](/docs/kubernetes/en/setup/best-practices/certificates/) for details.
- *Configure load balancer for apiserver*: Configure a load balancer
  ...
  [Create an External Load Balancer](/docs/kubernetes/en/tasks/access-application-cluster/create-external-load-balancer/)
  for details.
- *Separate and backup etcd service*: The etcd services can either run on the
  ...
  See [Operating etcd clusters for Kubernetes](/docs/kubernetes/en/tasks/administer-cluster/configure-upgrade-etcd/)
  and [Set up a High Availability etcd cluster with kubeadm](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/setup-ha-etcd-with-kubeadm/)
  for details.
...
- *Span multiple zones*: If keeping your cluster available at all times is
  ...
  See [Running in multiple zones](/docs/kubernetes/en/setup/best-practices/multiple-zones/) for details.
- *Manage on-going features*: If you plan to keep your cluster over time,
  ...
  [Certificate Management](/docs/kubernetes/en/tasks/administer-cluster/kubeadm/kubeadm-certs/)
  and [Upgrading kubeadm clusters](/docs/kubernetes/en/tasks/administer-cluster/kubeadm/kubeadm-upgrade/).
  See [Administer a Cluster](/docs/kubernetes/en/tasks/administer-cluster/)
...
and [kube-scheduler](/docs/kubernetes/en/reference/command-line-tools-reference/kube-scheduler/)
component pages. For highly available control plane examples, see
[Options for Highly Available topology](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/ha-topology/),
...
and [Operating etcd clusters for Kubernetes](/docs/kubernetes/en/tasks/administer-cluster/configure-upgrade-etcd/).
See [Backing up an etcd cluster](/docs/kubernetes/en/tasks/administer-cluster/configure-upgrade-etcd/#backing-up-an-etcd-cluster)
...
- *Configure nodes*: Nodes can be physical or virtual machines. If you want to
  ...
  [Node services](/docs/kubernetes/en/concepts/overview/components/#node-components). Consider:
  - The demands of your workloads when you set up nodes by having appropriate memory, CPU, and disk speed and storage capacity available.
  - Whether generic computer systems will do or you have workloads that need GPU processors, Windows nodes, or VM isolation.
- *Validate nodes*: See [Valid node setup](/docs/kubernetes/en/setup/best-practices/node-conformance/)
...
- *Add nodes to the cluster*: If you are managing your own cluster you can
  ...
  [Nodes](/docs/kubernetes/en/concepts/architecture/nodes/) section for information on how to set up Kubernetes to add nodes in these ways.
- *Scale nodes*: Have a plan for expanding the capacity your cluster will
  eventually need. See [Considerations for large clusters](/docs/kubernetes/en/setup/best-practices/cluster-large/)
...
- *Set up node health checks*: For important workloads, you want to make sure
  ...
  [Node Problem Detector](/docs/kubernetes/en/tasks/debug/debug-cluster/monitor-node-health/)
...
- *Authorization*: When you set out to authorize your regular users, you will probably choose
  between RBAC and ABAC authorization. See [Authorization Overview](/docs/kubernetes/en/reference/access-authn-authz/authorization/)
  to review different modes for authorizing user accounts (as well as service account access to
  your cluster):
  - *Role-based access control* ([RBAC](/docs/kubernetes/en/reference/access-authn-authz/rbac/)): Lets you
    ...
    to particular users.
  - *Attribute-based access control* ([ABAC](/docs/kubernetes/en/reference/access-authn-authz/abac/)): Lets you
    ...
    [Examples](/docs/kubernetes/en/reference/access-authn-authz/abac/#examples) for details.
...
- *Create user certificates and role bindings (RBAC)*: If you are using RBAC
  ...
  See [Certificate Signing Requests](/docs/kubernetes/en/reference/access-authn-authz/certificate-signing-requests/)
  for details.
- *Create policies that combine attributes (ABAC)*: If you are using ABAC
  ...
  [Examples](/docs/kubernetes/en/reference/access-authn-authz/abac/#examples).
- *Consider Admission Controllers*: Additional forms of authorization for
  ...
  [Webhook Token Authentication](/docs/kubernetes/en/reference/access-authn-authz/authentication/#webhook-token-authentication).
...
- *Set namespace limits*: Set per-namespace quotas on things like memory and CPU. See
  [Manage Memory, CPU, and API Resources](/docs/kubernetes/en/tasks/administer-cluster/manage-resources/)
...
- *Prepare for DNS demand*: If you expect workloads to massively scale up,
  ...
  [Autoscale the DNS service in a Cluster](/docs/kubernetes/en/tasks/administer-cluster/dns-horizontal-autoscaling/).
- *Create additional service accounts*: User accounts determine what users can
  ...
  See [Managing Service Accounts](/docs/kubernetes/en/reference/access-authn-authz/service-accounts-admin/)
  ...
  - Add secrets that a pod could use to pull images from a particular container registry. See
    [Configure Service Accounts for Pods](/docs/kubernetes/en/tasks/configure-pod-container/configure-service-account/)
    for an example.
  - Assign RBAC permissions to a service account. See
    [ServiceAccount permissions](/docs/kubernetes/en/reference/access-authn-authz/rbac/#service-account-permissions)
    for details.

## {{% heading "whatsnext" %}}

- Decide if you want to build your own production Kubernetes or obtain one from
  available [Turnkey Cloud Solutions](/docs/kubernetes/en/setup/production-environment/turnkey-solutions/)
  or [Kubernetes Partners](/partners/).
- If you choose to build your own cluster, plan how you want to
  handle [certificates](/docs/kubernetes/en/setup/best-practices/certificates/)
  and set up high availability for features such as
  [etcd](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/setup-ha-etcd-with-kubeadm/)
  and the
  [API server](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/ha-topology/).
- Choose from [kubeadm](/docs/kubernetes/en/setup/production-environment/tools/kubeadm/),
  [kops](/docs/kubernetes/en/setup/production-environment/tools/kops/) or
  [Kubespray](/docs/kubernetes/en/setup/production-environment/tools/kubespray/)
  deployment methods.
- Configure user management by determining your
  [Authentication](/docs/kubernetes/en/reference/access-authn-authz/authentication/) and
  [Authorization](/docs/kubernetes/en/reference/access-authn-authz/authorization/) methods.
- Prepare for application workloads by setting up
  [resource limits](/docs/kubernetes/en/tasks/administer-cluster/manage-resources/),
  [DNS autoscaling](/docs/kubernetes/en/tasks/administer-cluster/dns-horizontal-autoscaling/)
  and [service accounts](/docs/kubernetes/en/reference/access-authn-authz/service-accounts-admin/).
