...
- [ ] The [Role Based Access Control Good Practices](/docs/kubernetes/en/concepts/security/rbac-good-practices/)
...
A number of [Container Network Interface (CNI) plugins](/docs/kubernetes/en/concepts/extend-kubernetes/compute-storage-net/network-plugins/)
plugins provide the functionality to
restrict network resources that pods may communicate with. This is most commonly done
through [Network Policies](/docs/kubernetes/en/concepts/services-networking/network-policies/)
...
The [kubelet](/docs/kubernetes/en/reference/command-line-tools-reference/kubelet/) API access
...
and the [DenyServiceExternalIPs admission controller](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#denyserviceexternalips)
...
[cannot be granular enough to have authorization on the Pods' resources](/docs/kubernetes/en/concepts/security/rbac-good-practices/#workload-creation)
...
The [Pod Security Standards](/docs/kubernetes/en/concepts/security/pod-security-standards/)
...
[Pod Security](/docs/kubernetes/en/concepts/security/pod-security-admission/) admission,
...
[Pod Security](/docs/kubernetes/en/concepts/security/pod-security-admission/)
...
[Pod Security Standards](/docs/kubernetes/en/concepts/security/pod-security-standards/) set,
[can operate in several modes](/docs/kubernetes/en/concepts/security/pod-security-admission/#pod-security-admission-labels-for-namespaces),
`warn`, `audit` or `enforce` to gradually apply the most appropriate
[security context](/docs/kubernetes/en/tasks/configure-pod-container/security-context/)
...
For a hands-on tutorial on [Pod Security](/docs/kubernetes/en/concepts/security/pod-security-admission/),
see the blog post
[Kubernetes 1.23: Pod Security Graduates to Beta](/blog/2021/12/09/pod-security-admission-beta/).

[Memory and CPU limits](/docs/kubernetes/en/concepts/configuration/manage-resources-containers/)
...
[a security tutorial is available](/docs/kubernetes/en/tutorials/security/seccomp/). In
...
auditing through system logs. To [enable AppArmor in Kubernetes](/docs/kubernetes/en/tutorials/security/apparmor/),
...
[via their `securityContext` section](/docs/kubernetes/en/tasks/configure-pod-container/security-context/#assign-selinux-labels-to-a-container).
...
[Node Selectors](/docs/kubernetes/en/concepts/scheduling-eviction/assign-pod-node/)
...
[PodNodeSelector](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#podnodeselector)
admission controller.

[PodTolerationRestriction](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#podtolerationrestriction)
: An admission controller that allows administrators to restrict permitted
[tolerations](/docs/kubernetes/en/concepts/scheduling-eviction/taint-and-toleration/) within a
...
- [ ] [Bound service account token volume](/docs/kubernetes/en/reference/access-authn-authz/service-accounts-admin/#bound-service-account-token-volume)
...
be [encrypted at rest](/docs/kubernetes/en/tasks/administer-cluster/encrypt-data/).

Pods needing secrets should have these automatically mounted through volumes,
preferably stored in memory like with the [`emptyDir.medium` option](/docs/kubernetes/en/concepts/storage/volumes/#emptydir).
...
[`automountServiceAccountToken`](/docs/kubernetes/en/tasks/configure-pod-container/configure-service-account/#use-the-default-service-account-to-access-the-api-server)
...
[Bound Service Accounts](/docs/kubernetes/en/reference/access-authn-authz/service-accounts-admin/#bound-service-account-token-volume)
...
digital signature at deploy time [via admission control](/docs/kubernetes/en/tasks/administer-cluster/verify-signed-artifacts/#verifying-image-signatures-with-admission-controller).
...
[ephemeral debug container](/docs/kubernetes/en/tasks/debug/debug-application/debug-running-pod/#ephemeral-container)
...
The [Security Context](/docs/kubernetes/en/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-pod)
...
enforced via an [ImagePolicyWebhook](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#imagepolicywebhook).
Image signatures can also be automatically [verified with an admission controller](/docs/kubernetes/en/tasks/administer-cluster/verify-signed-artifacts/#verifying-image-signatures-with-admission-controller)
...
[enabled by default](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#which-plugins-are-enabled-by-default),
consider to leave them enabled unless you know what you are doing:

[`CertificateApproval`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#certificateapproval)
: Performs additional authorization checks to ensure the approving user has
permission to approve certificate request.

[`CertificateSigning`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#certificatesigning)
: Performs additional authorization checks to ensure the signing user has
permission to sign certificate requests.

[`CertificateSubjectRestriction`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#certificatesubjectrestriction)
...
[`MutatingAdmissionWebhook`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#mutatingadmissionwebhook)
: Allows the use of custom controllers through webhooks, these controllers may
mutate requests that it reviews.

[`PodSecurity`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#podsecurity)
: Replacement for Pod Security Policy, restricts security contexts of deployed
Pods.

[`ResourceQuota`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#resourcequota)
: Enforces resource quotas to prevent over-usage of resources.

[`ValidatingAdmissionWebhook`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#validatingadmissionwebhook)
...
[`DenyServiceExternalIPs`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#denyserviceexternalips)
...
[`AlwaysPullImages`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#alwayspullimages)
: Enforces the usage of the latest version of a tagged image and ensures that the deployer
has permissions to use the image.

[`ImagePolicyWebhook`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#imagepolicywebhook)
...
[`EventRateLimit`](/docs/kubernetes/en/reference/access-authn-authz/admission-controllers/#eventratelimit)
...
- [RBAC Good Practices](/docs/kubernetes/en/concepts/security/rbac-good-practices/) for
  further information on authorization.
- [Cluster Multi-tenancy guide](/docs/kubernetes/en/concepts/security/multi-tenancy/) for
